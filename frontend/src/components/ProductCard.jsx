        ...
        <div className="product-card">
            ...
            <div style={{ padding: '0.5rem 0' }}>
                ...
                <p style={{ color: 'var(--color-text-light)', fontSize: 'var(--text-sm)', marginBottom: '1rem' }}>
                    ...
                    {'⭐'.repeat(Math.round(product.rating))} ({product.numReviews} {t('reviews')})
                </p>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 'var(--text-xl)', fontWeight: 'bold', color: 'var(--color-primary-dark)' }}>
                        R$ {product.price.toFixed(2)}
                    </span>
                    <Link to={`/product/${product._id}`} className="btn btn-primary" style={{ padding: '0.5rem 1rem' }}>{t('view')}</Link>
                </div>
            </div>
        </div>
    );
